...
To support further exploration, `mfcurve` is generated using `plotly`, making the graph fully interactive. Users can hover over group means to access detailed information about group sizes, outcome values, and confidence intervals.
...
Consider a medical trial with five drugs (A-E), two doses (low vs. high), and two masking modes (single vs. double-blind). We simulate data for this 5 × 2 × 2 design:
...
df <- data.frame(
  ...
  masking = sample(c("single-blind", "double-blind"), 1000, replace = TRUE)
)
...
df$health <- 6 +
  ...
  ifelse(df$masking == "single-blind", 0.4, 0) +  # stronger effect for single-blind
...
(*Note*: setting the mode to "expanded" ensures an appropriate static layout; by default, `mfcurve` plots are interactive.)
```{r}
mfcurve(data = df, outcome = "health", factors = c("dose", "drug", "masking"), mode = "expanded")
```

```{r, include=FALSE, eval=FALSE}
# Export static graph for vignette
library(plotly)
mfcurve(df, outcome = "health", factors = c("dose", "drug", "masking"), mode = "expanded") %>%
  config(
...
Second, once we turn to the bottom panel, where each treatment is split into its binary components, we must look for horizontal patterns. Consider the first two rows, which indicate whether participants received a high or low drug dose. Scan these rows from left to right. No clear pattern emerges. Instead, dots appear to have been liberally sprinkled across the rows. In fact, if we cut the panel vertically in half and count dots per row, we realize that low and high doses are equally represented (i.e., five times each) on each side. 
...
Turn to the last set of rows to find another treatment effect. Although the pattern is slightly less clear, masking also seems to matter. Single-blind studies generally yield better health outcomes, possibly due to experimenter or confirmation bias. Double-blind studies, on the other hand, favor worse health outcomes. 
...
Finally, note that `mfcurve` returns a `plotly` object. This means you can customize the resulting graph using any standard `plotly` syntax. 

## Contributing
Feedback, contributions, and bug reports are welcome! Please fork this repository and submit a pull request with your changes.
